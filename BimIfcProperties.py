...
class BIM_IfcProperties:
    def GetResources(self):
        return {
            "Pixmap": os.path.join(
                os.path.dirname(__file__), "icons", "BIM_IfcProperties.svg"
            ),
    ...
    def Activated(self):
        ...
        self.form = FreeCADGui.PySideUic.loadUi(
            os.path.join(os.path.dirname(__file__), "dialogIfcProperties.ui")
        )
        ...
        QtCore.QObject.connect(
            self.form.onlyVisible, QtCore.SIGNAL("stateChanged(int)"), self.onVisible
        )
        QtCore.QObject.connect(
            self.form.onlySelected, QtCore.SIGNAL("stateChanged(int)"), self.onSelected
        )
        QtCore.QObject.connect(
            self.form.buttonBox, QtCore.SIGNAL("accepted()"), self.accept
        )
        QtCore.QObject.connect(
            self.form.onlyMatches, QtCore.SIGNAL("stateChanged(int)"), self.update
        )
        QtCore.QObject.connect(
            self.form.searchField,
        ...
        QtCore.QObject.connect(
            self.form.comboProperty,
            QtCore.SIGNAL("currentIndexChanged(int)"),
            self.addProperty,
        )
        QtCore.QObject.connect(
            self.form.comboPset, QtCore.SIGNAL("currentIndexChanged(int)"), self.addPset
        )
        QtCore.QObject.connect(
            self.form.buttonDelete, QtCore.SIGNAL("clicked()"), self.removeProperty
        )
    ...
    def onSelected(self, index):
        FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/BIM").SetInt(
            "IfcPropertiesSelectedState", index
        )
        self.objectslist, searchterms = self.rebuildObjectsList()
    ...
    def onVisible(self, index):
        FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/BIM").SetInt(
            "IfcPropertiesVisibleState", index
        )
        self.update()
