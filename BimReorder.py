...
class BIM_Reorder:
    def GetResources(self):
        return {
            "Pixmap": os.path.join(
                os.path.dirname(__file__), "icons", "BIM_Reorder.svg"
            ),
...
class BIM_Reorder_TaskPanel:
    def __init__(self, obj):
        ...
        self.form = FreeCADGui.PySideUic.loadUi(
            os.path.join(os.path.dirname(__file__), "dialogReorder.ui")
        )
        self.form.setWindowIcon(
            QtGui.QIcon(
                os.path.join(os.path.dirname(__file__), "icons", "BIM_Reorder.svg")
            )
        )
