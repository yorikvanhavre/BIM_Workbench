...
class BIM_Material:
    def GetResources(self):
        return {
            "Pixmap": os.path.join(
                os.path.dirname(__file__), "icons", "BIM_Material.svg"
            ),
    ...
    def Activated(self):
        ...
        if matList.count():
            ...
            QtCore.QObject.connect(
                context1, QtCore.SIGNAL("triggered()"), self.onStartRename
            )
            context2 = self.contextMenu.addAction(translate("BIM", "Duplicate"))
            QtCore.QObject.connect(
                context2, QtCore.SIGNAL("triggered()"), self.onDuplicate
            )
            context3 = self.contextMenu.addAction(translate("BIM", "Merge to..."))
            QtCore.QObject.connect(
                context3, QtCore.SIGNAL("triggered()"), self.onMergeTo
            )
            context4 = self.contextMenu.addAction(translate("BIM", "Delete"))
            QtCore.QObject.connect(
                context4, QtCore.SIGNAL("triggered()"), self.onDelete
            )
