...
class Opening(ShapeGroup, IfcProduct):
    ...
    def set_properties(self, obj):
        ...
        _tip = (
            "List of available shapes for the opening Additions.\n"
            "Chose Custom to use the Addition Elements objects shape."
        )
        obj.addProperty(
            "App::PropertyEnumeration", "Addition", "Component - Additions", _tip
        ).Addition = ["None", "Default Sill", "Custom"]
        ...
        _tip = (
            "Chose if the Addition Elements have to be individually displayed or\n"
            "merged into the Opening object shape."
        )
        obj.addProperty(
            "App::PropertyEnumeration", "AdditionMode", "Component - Additions", _tip
        ).AdditionMode = ["Embed Shape", "Display Children"]

        # COMPONENTS - FILLING Properties (Windows, doors) ----------------------------
        _tip = (
            "List of available shapes for the Fill element.\n"
            "Chose Custom to use the Fill Element object shape."
        )
        obj.addProperty(
            "App::PropertyEnumeration", "Fill", "Component - Filling", _tip
        ).Fill = ["None", "Preset Door", "Preset Window", "By Sketch", "Custom"]

        _tip = "Alignment of the Fill Element according to the Host Thickness property."
        obj.addProperty(
            "App::PropertyEnumeration", "FillAlignment", "Component - Filling", _tip
        ).FillAlignment = ["Left", "Center", "Right"]
        obj.FillAlignment = "Right"

        _tip = "Offset of the Fill Element from the chosen Fill Alignment."
        obj.addProperty(
            "App::PropertyDistance", "FillDisplacement", "Component - Filling", _tip
        ).FillDisplacement = 0.0

        _tip = "Link the door or the window that you want to insert into the opening."
        obj.addProperty(
            "App::PropertyLinkGlobal", "FillElement", "Component - Filling", _tip
        )

        _tip = (
            "Chose if the Fill Element have to be individually displayed or\n"
            "merged into the Opening object shape."
        )
        obj.addProperty(
            "App::PropertyEnumeration", "FillMode", "Component - Filling", _tip
        ).FillMode = ["Embed Shape", "Display Child"]

        # COMPONENTS - VOID Properties (not implemented yet) ----------------------------
        _tip = (
            "List of available shapes of the Opening void.\n"
            "Chose Custom to use the Void Element object shape."
        )
        obj.addProperty(
            "App::PropertyEnumeration", "Void", "Component - Void", _tip
        ).Void = ["Rectangular", "Arc", "Custom"]

        _tip = (
            "Ponter to the object that will be used to cut the wall.\n"
            "To use it, set Void property to Custom"
        )
        obj.addProperty(
            "App::PropertyLinkGlobal", "VoidElement", "Component - Void", _tip
        )

        _tip = "Subtract also positive shapes from the wall."
        obj.addProperty(
            "App::PropertyBool", "VoidSubtractAll", "Component - Void", _tip
        ).VoidSubtractAll = False

        _tip = "This property stores the shape of the Void object to be subtracted from the host."
        obj.addProperty("Part::PropertyPartShape", "VoidShape", "Base", _tip)

        # Geometry Properties (not implemented yet) ----------------------------
        _tip = "Architectural Width of the opening object"
        obj.addProperty(
            "App::PropertyLength", "OpeningWidth", "Geometry", _tip
        ).OpeningWidth = 800
        _tip = "Architectural Height of the opening object"
        obj.addProperty(
            "App::PropertyLength", "OpeningHeight", "Geometry", _tip
        ).OpeningHeight = 1500
        _tip = (
            "Thickness of the hosted object.\n"
            "This property is set by the hosting wall on creation"
        )
        obj.addProperty(
            "App::PropertyLength", "HostThickness", "Geometry", _tip
        ).HostThickness = 500
        _tip = "Propagate geometry properties to opening childrens"
        obj.addProperty(
            "App::PropertyBool", "PropagateGeometry", "Geometry", _tip
        ).PropagateGeometry = False
    ...
    def sync_filling_preset_subproperties(self, obj):
        ...
        for property in obj.PropertiesList:
            if (
                obj.getGroupOfProperty(property) != "Component - Filling - Options"
                or property == "FillType"
            ):
                continue
            obj.removeProperty(property)
    ...
    def get_default_sill_shape(self, obj):
        return window_presets.default_sill(
            opening_width=obj.OpeningWidth.Value,
    ...
    def get_fill_shape(self, obj):
        ...
        if "Fill" in obj.PropertiesList and obj.Fill == "None":
            ...
            if "FillElement" in obj.PropertiesList and obj.FillElement:
                if (
                    "Shape" in obj.FillElement.PropertiesList
                    and not obj.FillElement.Shape.isNull()
                ):
                    if (
                        "FillMode" in obj.PropertiesList
                        and obj.FillMode == "Embed Shape"
                    ):
                        # if FillMode is "Embed Shape" look for a shape to return
                        f_shape = obj.FillElement.Shape.copy()
                    elif (
                        "FillMode" in obj.PropertiesList
                        and obj.FillMode == "Display Child"
                    ):
    ...
    def get_preset_door_shape(self, obj):
        ...
        if (
            not "OpeningWidth" in obj.PropertiesList
