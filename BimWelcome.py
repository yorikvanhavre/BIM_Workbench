...
class BIM_Welcome:
    def GetResources(self):
        return {
            "Pixmap": os.path.join(
                os.path.dirname(__file__), "icons", "BIM_Welcome.svg"
            ),
    ...
    def Activated(self):
        ...
        self.form = FreeCADGui.PySideUic.loadUi(
            os.path.join(os.path.dirname(__file__), "dialogWelcome.ui")
        )

        # set the title image
        self.form.image.setPixmap(
            QtGui.QPixmap(
                os.path.join(os.path.dirname(__file__), "icons", "banner.png")
            )
        )

        # handle the tutorial links
        QtCore.QObject.connect(
            self.form.label_4, QtCore.SIGNAL("linkActivated(QString)"), self.handleLink
        )
        QtCore.QObject.connect(
            self.form.label_7, QtCore.SIGNAL("linkActivated(QString)"), self.handleLink
        )
